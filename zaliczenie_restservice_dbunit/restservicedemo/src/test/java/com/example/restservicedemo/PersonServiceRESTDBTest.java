...
public class PersonServiceRESTDBTest {
	...
	Chair aChair = new Chair("Jeep", 1975, aPerson);
	...
	@Test
	public void addChair() throws Exception{
	
		
		given().contentType(MediaType.APPLICATION_JSON).body(aChair)
				.when().post("/chair/add").then().assertThat().statusCode(201);
		
		IDataSet dbDataSet = connection.createDataSet();
		ITable actualTable = dbDataSet.getTable("CHAIR");
		ITable filteredTable = DefaultColumnFilter.excludedColumnsTable
				(actualTable, new String[]{"C_ID"});
		
		IDataSet expectedDataSet = new FlatXmlDataSetBuilder().build(
				new File("src/test/resources/chairData.xml"));
		ITable expectedTable = expectedDataSet.getTable("CHAIR");
